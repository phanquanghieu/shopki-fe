  ...
  const handleAddToCart = async () => {
    try {
      ...
      if (guest) {
        ...
        return helper.toast('success', 'Thêm vào giỏ hàng nhóm thành công')
      }

      if (!groupCartId) {
        if (!user) return helper.toast('warning', 'Hãy đăng nhập!!!')
        let res = await request.post('/api/groupcart/get', { id: user.id, phone: user.phone })
      ...
      helper.toast('success', 'Thêm vào giỏ hàng thành công')
    } catch (error) {}
  }
  ...
  return (
    <div className='h-product-item'>
      ...
      <div className='h-p-item-container'>
        <div className='h-addcart' onClick={handleAddToCart}>
          <i className='fas fa-cart-arrow-down'></i>
          <div>Add to Group Cart</div>
        </div>
        <div className='h-p__img'>
          <img
            src={
              props.product.imageUrl ||
              'https://cf.shopee.vn/file/570cf67f80a87870ebb065c330e344ca_tn'
            }
            alt=''
          />
